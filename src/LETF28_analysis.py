...
# Block size for bootstrap (in trading days)
# 63 days ≈ 3 months - preserves multi-month momentum patterns
# 
# WHY 63 INSTEAD OF 20:
# TQQQ is extremely path-dependent. A sustained 3-month rally compounds
# very differently than three 1-month rallies shuffled randomly.
# Larger blocks preserve the multi-month trends that make leveraged
# ETFs either soar (sustained bull) or crash (sustained bear).
#
# With real QQQ data from 1971+ (~54 years = ~13,600 days):
#   Block count with 50% overlap: ~(13,600 - 63) / 31 ≈ 436 blocks
#   That's still plenty for bootstrap sampling.
#
# Trade-off: Larger blocks = fewer unique blocks = less path diversity.
# But for TQQQ accuracy, preserving momentum matters more.
...
def fetch_historical_data():
    ...
    # NASDAQ (didn't exist before 1971)
    if 'NASDAQ_Ret' not in df.columns or df['NASDAQ_Ret'].isna().any():
        # Approximate NASDAQ as 1.25x S&P 500 (historical beta)
        df['NASDAQ_Ret'] = df['NASDAQ_Ret'].fillna(df['SPY_Ret'] * 1.25)
        print("  ✓ NASDAQ approximated for pre-1971 period (1.25x SPY)")
    
    # QQQ (didn't exist before 1999)
    if 'QQQ_Ret' not in df.columns or df['QQQ_Ret'].isna().any():
        df['QQQ_Ret'] = df['QQQ_Ret'].fillna(df['NASDAQ_Ret'])
        print("  ✓ QQQ approximated for pre-1999 period")
    ...
    # Interest rates
    if 'IRX' not in df.columns or df['IRX'].isna().any():
        # Use Fama-French RF if available, else default
        if 'RF' in df.columns:
            df['IRX'] = df['IRX'].fillna(df['RF'] * 252 * 100)  # Convert daily to annual %
        df['IRX'] = df['IRX'].fillna(4.5)
        print("  ✓ Interest rates filled from RF or default")
    
    if 'Cash_Ret' not in df.columns:
        df['Cash_Ret'] = df['IRX'] / 100 / 252
    
    # Treasury returns
    if 'TLT_Ret' not in df.columns or df['TLT_Ret'].isna().any():
        # Approximate: negative correlation to stocks + risk-free return
        rf_daily = df['IRX'] / 100 / 252
        df['TLT_Ret'] = df['TLT_Ret'].fillna(df['SPY_Ret'] * -0.25 + rf_daily)
        print("  ✓ Treasury returns approximated for older periods")
...
class BlockBootstrapReturns:
    ...
    def __init__(self, df: pd.DataFrame, block_size: int = 20):
        ...
        print(f"     Low vol blocks: {len(self.regime_blocks[0])}")
        print(f"     High vol blocks: {len(self.regime_blocks[1])}")
    
    def _create_regime_blocks(self) -> Dict[int, List[np.ndarray]]:
        """
        Create pools of return blocks separated by regime.
        
        IMPORTANT: QQQ blocks only use REAL QQQ data (1999+).
        SPY/TLT/VIX use all available data for maximum coverage.
        
        We maintain TWO pools:
        1. Full blocks (SPY, QQQ, TLT, VIX) — only from periods with real QQQ
        2. SPY-only blocks — from all periods (used as fallback and for 
           strategies that don't need QQQ)
        
        This prevents synthetic QQQ data (which is just SPY*1.25) from 
        contaminating the TQQQ simulation.
        
        Returns:
            Dict mapping regime_id -> list of return blocks (numpy arrays)
        """
        regime_blocks = {0: [], 1: []}
        
        # Also create SPY-only blocks for maximum historical coverage
        self.spy_only_regime_blocks = {0: [], 1: []}
        ...
        # Get QQQ and TLT returns
        if 'QQQ_Ret' in self.df.columns:
            qqq_ret = self.df['QQQ_Ret'].values
        else:
            qqq_ret = spy_ret * 1.25  # Approximate (only used in SPY-only blocks)
        
        if 'TLT_Ret' in self.df.columns:
            tlt_ret = self.df['TLT_Ret'].values
        else:
            tlt_ret = spy_ret * -0.25  # Approximate
        
        # Get IRX (interest rates) — this is the key addition!
        # IRX is in percentage points (e.g., 4.5 means 4.5%)
        # We bootstrap it alongside returns so that borrow costs
        # match the rate environment of each sampled block.
        if 'IRX' in self.df.columns:
            irx = self.df['IRX'].values
        else:
            irx = np.full(len(self.df), 4.5)  # Fallback default
        
        # Determine which dates have REAL QQQ data (not synthetic)
        # QQQ ETF launched March 1999. NASDAQ composite available from 1971.
        # We use 1971 as cutoff — NASDAQ is close enough to QQQ for bootstrap
        # (both are cap-weighted tech-heavy indices)
        REAL_QQQ_CUTOFF = pd.Timestamp('1971-01-01')
        ...
        real_qqq_count = {0: 0, 1: 0}
        spy_only_count = {0: 0, 1: 0}
        
        for start_idx in range(0, n_days - block_size + 1, block_size // 2):
            # Overlapping blocks (50% overlap) for more samples
            end_idx = start_idx + block_size
            
            # Determine regime for this block (majority vote from VIX)
            block_vix = vix[start_idx:end_idx]
            regime = 0 if np.nanmedian(block_vix) < 25 else 1
            
            # Extract returns for this block
            block_spy = spy_ret[start_idx:end_idx]
            block_qqq = qqq_ret[start_idx:end_idx]
            block_tlt = tlt_ret[start_idx:end_idx]
            block_vix_vals = block_vix
            
            # Skip blocks with too many NaNs
            if np.isnan(block_spy).sum() > block_size // 4:
                continue
            
            # Extract IRX for this block
            block_irx = irx[start_idx:end_idx]
            
            # 5 columns: SPY, QQQ, TLT, VIX, IRX
            block_data = np.column_stack([
                np.nan_to_num(block_spy, nan=0),
                np.nan_to_num(block_qqq, nan=0),
                np.nan_to_num(block_tlt, nan=0),
                np.nan_to_num(block_vix_vals, nan=20),
                np.nan_to_num(block_irx, nan=4.5)
            ])
            
            # Check if this block has REAL QQQ data
            # Block qualifies if ALL days in the block have real QQQ
            block_has_real_qqq = has_real_qqq[start_idx:end_idx].all()
            
            if block_has_real_qqq:
                # Full block with real QQQ — goes in primary pool
                regime_blocks[regime].append(block_data)
                real_qqq_count[regime] += 1
            
            # ALL blocks go in the SPY-only pool (for non-TQQQ strategies 
            # and as fallback)
            spy_only_block = np.column_stack([
                np.nan_to_num(block_spy, nan=0),
                np.nan_to_num(block_spy, nan=0) * 1.25,  # Placeholder QQQ
                np.nan_to_num(block_tlt, nan=0),
                np.nan_to_num(block_vix_vals, nan=20),
                np.nan_to_num(block_irx, nan=4.5)
            ])
            self.spy_only_regime_blocks[regime].append(spy_only_block)
            spy_only_count[regime] += 1
        
        print(f"     Real QQQ blocks: Low vol={real_qqq_count[0]}, High vol={real_qqq_count[1]}")
        print(f"     SPY-only blocks: Low vol={spy_only_count[0]}, High vol={spy_only_count[1]}")
        
        return regime_blocks
    
    def sample_block(self, regime: int, rng: np.random.Generator = None) -> np.ndarray:
        """
        ...
        if len(blocks) == 0:
            # Fallback: generate synthetic block
    ...
    def sample_returns(self, n_days: int, regime_path: np.ndarray,
        ...
        """
        Generate a full return series by sampling blocks.
        
        This is the main method for generating simulation returns.
        
        Args:
            n_days: Number of days to generate
            regime_path: Array of regime IDs (0 or 1) for each day
            rng: Random number generator
            add_student_t_noise: Whether to add Student-t noise
            bootstrap_weight: Weight for bootstrap vs noise (0-1)
        
        Returns:
            Dict with keys 'SPY_Ret', 'QQQ_Ret', 'TLT_Ret', 'VIX'
        """
        if rng is None:
            rng = np.random.default_rng()
        ...
        # Sample blocks and stitch them together
        current_day = 0
        
        while current_day < n_days:
            # Get regime for this block (use majority of remaining days needed)
            remaining = n_days - current_day
            block_len = min(self.block_size, remaining)
            
            # Determine which regime to use for this block
            block_regime_path = regime_path[current_day:current_day + block_len]
            regime = int(np.median(block_regime_path))
            
            # Sample a block
            block = self.sample_block(regime, rng)
            
            # Truncate if needed
            block = block[:block_len]
            
            # Place in output arrays
            spy_returns[current_day:current_day + block_len] = block[:, 0]
            qqq_returns[current_day:current_day + block_len] = block[:, 1]
            tlt_returns[current_day:current_day + block_len] = block[:, 2]
            vix_series[current_day:current_day + block_len] = block[:, 3]
            irx_series[current_day:current_day + block_len] = block[:, 4]
            
            current_day += block_len
        
        # Add Student-t noise for additional variation
        if add_student_t_noise and bootstrap_weight < 1.0:
            noise_weight = 1.0 - bootstrap_weight
            
            # Regime-dependent noise scaling
            # Slightly reduced from previous values because:
            # 1. Fix #1 (borrow cost bug) adds ~10.5% annual drag back
            # 2. Fix #2 (real QQQ blocks) adds proper tech volatility
            # 3. Correlated noise (below) preserves variance better
            noise_scale_spy = np.where(regime_path == 0, 0.007, 0.022)
            noise_scale_qqq = noise_scale_spy * 1.35  # Reduced: real QQQ blocks now carry proper vol
            noise_scale_tlt = noise_scale_spy * 0.5
            
            # ============================================================
            # CORRELATED Student-t noise using Cholesky decomposition
            # ============================================================
            # Instead of independent noise, we generate correlated noise
            # that respects the QQQ-SPY and TLT-SPY correlation structure.
            #
            # How it works (simplified):
            # 1. Generate 3 independent random numbers
            # 2. Mix them together using the correlation matrix
            # 3. Result: correlated random numbers with proper relationships
            #
            # This means even the "noise" component has QQQ moving mostly
            # with SPY and TLT moving mostly opposite, as in real markets.
            
            # Generate INDEPENDENT Student-t innovations first
            independent_noise = rng.standard_t(
                df=STUDENT_T_DF, size=(n_days, 3)
            )
            
            # Apply correlation structure day-by-day based on regime
            # Correlation matrices: [SPY, QQQ, TLT]
            corr_low_vol = np.array([
                [1.000, 0.835, -0.207],   # SPY correlations
                [0.835, 1.000, -0.150],    # QQQ correlations
                [-0.207, -0.150, 1.000]    # TLT correlations
            ])
            corr_high_vol = np.array([
                [1.000, 0.950, -0.447],    # Crisis: equities highly correlated
                [0.950, 1.000, -0.400],    # Crisis: QQQ-SPY almost 1:1
                [-0.447, -0.400, 1.000]    # Crisis: flight to quality
            ])
            
            # Cholesky decomposition (the "mixing matrix")
            # This is a standard technique — think of it as a recipe for
            # combining independent random numbers into correlated ones
            ...
            # Blend bootstrap returns with correlated noise
            spy_returns = bootstrap_weight * spy_returns + noise_weight * spy_noise
...
def generate_fat_tailed_returns(n_days: int, regime_path: np.ndarray,
    ...
    # Add VIX dynamics (AR(1) with shocks)
    for t in range(1, n_days):
        ...
        vix_series[t] = (0.88 * vix_series[t-1] + 
                        0.12 * target_vix + 
                        vix_jump + 
                        rng.normal(0, 1.5))
    ...
    for regime_id in range(N_REGIMES):
        ...
        n_regime_days = mask.sum()
        if n_regime_days > 0:
            irx_series[mask] = irx_base_map[regime_id] + np.random.normal(0, 0.5, n_regime_days)
...
def simulate_single_path_fixed(args):
    ...
    regime_path_full[0] = start_regime  # Use randomized starting regime!
    days_in_current_regime = 0
    
    for t in range(1, total_days_to_generate):
        days_in_current_regime += 1
        current_regime = regime_path_full[t-1]
        
        # Check minimum duration
        if days_in_current_regime < MIN_REGIME_DURATION[current_regime]:
            regime_path_full[t] = current_regime
            continue
...
# ========================================================================
    ...
    if bootstrap_vix is not None and USE_BLOCK_BOOTSTRAP:
        ...
        for t in range(1, sim_days):
            ...
            # AR(1) component
            ar1_vix = 0.88 * vix[t-1] + 0.12 * vix_base[regime]
            ...
            vix[t] = 0.6 * hist_vix + 0.4 * (ar1_vix + vix_jump) + np.random.normal(0, 1.0)
            vix[t] = max(10, vix[t])
    else:
