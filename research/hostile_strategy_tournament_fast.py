...
def s8_composite(px,r1,r3,rf):
    ma=base.rm2(px,200)
    delta=np.diff(px,axis=1,prepend=px[:,:1]); gain=np.maximum(delta,0); loss=np.maximum(-delta,0)
    ag=base.rm2(gain,14); al=base.rm2(loss,14); rs=ag/np.maximum(al,1e-12); rsi=100-100/(1+rs)
    vix=base.rs2(r1,20)*math.sqrt(base.TD)*100
    score=np.zeros_like(px,dtype=np.int8)
    ...
    state=np.where(score==3,2,np.where(score==2,1,0)).astype(np.int8)
    ret=np.where(state==2,r3,np.where(state==1,r1,rf)); sw=(np.diff(state,axis=1,prepend=np.zeros((state.shape[0],1)))!=0)
    return ret-base.TC_SWITCH*sw

def s19_roth(px,r1,r3,rf):
    mom=base.rm2(r1,126)*126
    neg=np.where(r3<0,r3,0); dvol=base.rs2(neg,20)*math.sqrt(base.TD)
    vf=base.rs2(r3,5)*math.sqrt(base.TD); vs=base.rs2(r3,60)*math.sqrt(base.TD); sma=base.rm2(px,100)
    want=np.zeros_like(px)
    ...
    momscore=np.where(mm>.15,1.0,np.where(mm>.05,.7,np.where(mm>0,.4,0)))
    trend=np.where(p>sm,.5,0); vr=vfast/np.maximum(vslow,1e-12); volscore=np.where(vr<.9,.5,np.where(vr<1.2,.3,0))
    conviction=momscore+trend+volscore; mult=.3+conviction*.55; alloc=np.clip((.32/np.maximum(dv,1e-12))*mult,0,1.0)
    want[:,1:]=np.where(valid,alloc,0)
    a=hysteresis_fast(want,.05)
    return base.alloc_returns(a,r3,rf)

orig_build=base.build_strategies

def build_extended(bm,br):
    out=orig_build(bm,br)
    r1=base.levret(bm,br,1); r3=base.levret(bm,br,3); px=np.cumprod(np.maximum(1+r1,1e-12),axis=1); ma=base.rm2(px,200)
    # Exact repo S5: same -2% threshold for both entering and exiting, not true +/-2 hysteresis.
...
base.build_strategies=build_extended
base.STRATEGIES=base.STRATEGIES+('repo_s5_minus2_threshold','true_sma_plus2_minus2_band','repo_s8_composite','s19_roth_cap100')

if __name__=='__main__':
    x=np.zeros((2,20)); h=np.array([5,10],dtype=np.int64)
    pathstats_fast(x,h); hysteresis_fast(x,.05); weekly_fast(x,5); true_band_state(x,x+1,.02,.02)
    base.main()
