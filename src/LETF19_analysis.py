...
def calculate_comprehensive_tax_v6(
    ...
    ltcg_brackets = LTCG_BRACKETS_2024.get(filing_status, LTCG_BRACKETS_2024['single'])  # If LTCG is used
    state_brackets = STATE_TAX_BRACKETS['CA'].get(filing_status, STATE_TAX_BRACKETS['CA']['single'])  # If state tax
    state_std_ded = STATE_TAX_BRACKETS['CA']['std_deduction'].get(filing_status, STATE_TAX_BRACKETS['CA']['std_deduction']['single'])  # If state tax
...
class TradeJournal:
    ...
    def log_allocation_change(self, day: int, asset: str, 
        ...
        """
        FIXED: Now correctly calculates share-based trades.
        
        Old bug: Used delta_alloc * portfolio_value
        This created mismatches because portfolio_value != share_value
        
        Fix: Calculate actual shares before/after, then dollar amount from that
        """
        if price <= 0:
            return
        
        # Calculate ACTUAL shares held before and after allocation change
        prev_shares = (prev_allocation * portfolio_value) / price
        new_shares = (new_allocation * portfolio_value) / price
        
        share_change = new_shares - prev_shares
        
        # Skip negligible changes
        if abs(share_change) < 0.001:
            return
        
        # Determine action and recalculate dollar amount from shares
        if share_change > 0:
            action = 'BUY'
            dollar_amount = share_change * price
        else:
            action = 'SELL'
            dollar_amount = abs(share_change) * price
...
def create_summary_statistics(mc_results, time_horizon):
    ...
    for sid in taxable_ids:
        ...
        results = mc_results[sid]
        wealth = np.array([r['Final_Wealth'] for r in results])
    # FIXED: Calculate post-tax wealth for ALL simulations first
        # Then take percentiles of the post-tax distribution
        # This prevents the bug where P75 pre-tax might be from a volatile path
        # with massive realized gains, causing impossible post-tax results
        ...
        for sim_result in results:
            ...
            if trade_list and len(trade_list) > 0:
                tax_result = process_trades_with_wired_engine(
                    ...
                    debug=False,  # Disable debug for bulk processing
...
def process_trades_with_wired_engine(
    ...
    if tax_config is None:
        tax_config = {'filing_status': 'single', 'state': 'CA'}
    ...
    for year in range(time_horizon_years):
        ...
        # Use the ordinary income from tax_config (user's actual W-2 income)
        assumed_ordinary_income = tax_config.get('ordinary_income', 150000)
        ...
        # Tax with ordinary income + capital gains
