...
#   'historical'  → empirically anchored; drift_tilt auto-computed to hit
#                   HISTORICAL_NOMINAL_CAGR_TARGET; dividend yield & inflation
#                   added.  Use for: long-run empirical projections.
...
#
# Set MARKET_SCENARIO to one of: 'neutral', 'historical', 'pessimistic', 'optimistic'
...
_SCENARIO_PRESETS = {
    'neutral': {
        ...
        'drift_tilt':         0.0,
        # crisis_prob_scale: multiplier on calm→crisis transition probability TM[0,1].
        # Row 0 is renormalised (Markov-consistent); row 1 unchanged.
        # 2.0 → twice the crisis entry frequency; 0.5 → half.
    ...
    'historical': {
        # drift_tilt is intentionally 0.0 here; compute_historical_drift_anchor()
        # replaces it at runtime with the analytically computed anchor value.
        # DO NOT hard-code a non-zero value here — it will be overwritten.
        'drift_tilt':         0.0,    # overridden by compute_historical_drift_anchor()
        'crisis_prob_scale':  1.0,
        'div_yield':          HISTORICAL_DIV_YIELD,   # 1.5%
        'inflation':          HISTORICAL_INFLATION,   # 3.0%
    },
    'pessimistic': {
        'drift_tilt':        -0.02,    # −2%/yr intentional overlay
        'crisis_prob_scale':  2.0,     # 2× crisis entry probability
        'div_yield':          0.015,
        'inflation':          0.04,
    },
    'optimistic': {
        'drift_tilt':        +0.02,    # +2%/yr intentional overlay
        'crisis_prob_scale':  0.5,     # 0.5× crisis entry probability
        'div_yield':          0.020,
        'inflation':          0.02,
    },
}
...
def compute_historical_drift_anchor(bypass_cache: bool = False) -> float:
    ...
    The mu used here mirrors the crisis drift constraint (mu_crisis ≤ 0 for equity)
    so the anchor is consistent with the post-constraint calibrated drift.
    ...
    # Mirror crisis drift constraint: equity crisis mu ≤ 0
    mu1_spy = min(mu1_raw, min(mu0_spy, 0.0))
    ...
    drift_tilt = HISTORICAL_NOMINAL_CAGR_TARGET - natural_cagr - HISTORICAL_DIV_YIELD
    _HISTORICAL_DRIFT_ANCHOR = float(np.clip(drift_tilt, -0.10, +0.10))

    print(f"  [historical anchor] natural_CAGR≈{natural_cagr:.2%}  "
          f"target={HISTORICAL_NOMINAL_CAGR_TARGET:.2%}  "
...
def get_scenario() -> dict:
    ...
    'historical'  → empirically anchored to HISTORICAL_NOMINAL_CAGR_TARGET;
                    drift_tilt auto-computed; includes dividend yield overlay.
    'pessimistic' → intentional −2% drift overlay, 2× crisis entry probability.
    'optimistic'  → intentional +2% drift overlay, 0.5× crisis entry probability.

    For 'historical', drift_tilt is injected at runtime — the preset value (0.0)
    is overridden by compute_historical_drift_anchor() to close the gap between
    the model's natural calibrated CAGR and HISTORICAL_NOMINAL_CAGR_TARGET.
    """
    sc = _SCENARIO_PRESETS.get(MARKET_SCENARIO, _SCENARIO_PRESETS['neutral'])
    if MARKET_SCENARIO == 'historical':
        anchor = compute_historical_drift_anchor()
        sc = dict(sc)           # shallow copy — never mutate the preset dict
        sc['drift_tilt'] = anchor
    return sc
...
def calibrate_joint_return_model(df: pd.DataFrame, regimes: np.ndarray) -> Dict:
    ...
    for regime in range(N_REGIMES):
        ...
        if len(reg_df) < 80:
            ...
            # nu ≥ 6 ensures finite 4th moment and tractable GARCH constraint
            nu = 8.0 if regime == 0 else 6.0
            garch_alpha = 0.05 if regime == 0 else 0.08
            garch_gamma = 0.05 if regime == 0 else 0.04   # typical equity leverage effect
            ...
            dcc_b = 0.92 if regime == 0 else 0.88
        else:
            arr = reg_df.values
            ...
            _prior_corr = _prior_corr_r0 if regime == 0 else _prior_corr_r1
            _shrink = 0.25 if regime == 0 else 0.40
            _blended = (1.0 - _shrink) * _sample_corr + _shrink * _prior_corr
            cov = nearest_psd_matrix(_blended) * np.outer(_sigs, _sigs)

            # Tail heaviness: nu estimated from empirical kurtosis.
            # Clamped to [6.0, 15.0] — ensures:
            #   (a) finite 4th moment with margin (nu > 4),
            #   (b) realistic daily kurtosis (6/(nu-4) ≤ 3 for nu ≥ 6),
            #   (c) eff_alpha scaling ν/(ν-2) ≤ 1.50, tractable GARCH constraint.
            k = np.nanmean([stats.kurtosis(reg_df[c], fisher=False, nan_policy='omit') for c in assets])
            if np.isfinite(k) and k > 3.05:
                nu = float(np.clip(4 + 6 / (k - 3 + 1e-6), 6.0, 15.0))
            else:
                nu = 8.0
    ...
    # ── Economic constraint: crisis regime drift ≤ calm regime drift ──────────
    # In real markets crisis regimes feature elevated vol and flat/negative drift.
    # A positive crisis-mu relative to calm-mu is a sample-period artifact:
    # post-crash recoveries and bear-market bounces are counted inside crisis
    # regime windows, inflating the crisis conditional mean.
    #
    # Without this constraint the scenario sensitivity produces the paradox
    # "more crises = higher CAGR" (observed when crisis_prob_scale > 1 and
    # mu_crisis > mu_calm).
    #
    # Rule: for equity assets (SPY=0, QQQ=1), if mu_crisis > mu_calm,
    #       clamp mu_crisis = min(mu_calm, 0.0)  — at most zero, never positive.
    # TLT (index 2) is exempt — flight-to-quality legitimately produces
    # positive crisis drift in bonds.
    if 0 in model['regimes'] and 1 in model['regimes']:
        mu_calm_arr    = np.asarray(model['regimes'][0]['mu'], dtype=float).copy()
        mu_crisis_pre  = np.asarray(model['regimes'][1]['mu'], dtype=float).copy()
        mu_crisis_arr  = mu_crisis_pre.copy()
        _EQUITY_IDX    = [0, 1]   # SPY=0, QQQ=1; TLT=2 exempt
        _clamped       = False
        for _i in _EQUITY_IDX:
            if mu_crisis_arr[_i] > mu_calm_arr[_i]:
                mu_crisis_arr[_i] = min(mu_calm_arr[_i], 0.0)
                _clamped = True
        model['regimes'][1]['mu'] = mu_crisis_arr
        if _clamped:
            print(f"  [crisis drift constraint applied — sample-period bias removed]")
            print(f"    SPY: mu_calm={mu_calm_arr[0]:+.5f}  "
                  f"mu_crisis_raw={mu_crisis_pre[0]:+.5f}  "
                  f"mu_crisis_clamped={mu_crisis_arr[0]:+.5f}")
            print(f"    QQQ: mu_calm={mu_calm_arr[1]:+.5f}  "
                  f"mu_crisis_raw={mu_crisis_pre[1]:+.5f}  "
                  f"mu_crisis_clamped={mu_crisis_arr[1]:+.5f}")
        else:
            print(f"  [crisis drift constraint] no change — mu_crisis already ≤ mu_calm")
...
def simulate_joint_returns_t(n_days: int, regime_path: np.ndarray, joint_model: Dict,
    ...
    for t in range(n_days):
        ...
        if eff_alpha_gjr + beta >= 1.0:
            beta = max(0.70, 0.98 - eff_alpha_gjr)
...
def calibrate_funding_spread_model(df: pd.DataFrame, bypass_cache: bool = False) -> Dict[str, float]:
    ...
    vix = df['VIX'].fillna(method='ffill').fillna(method='bfill').fillna(20.0).to_numpy(dtype=float)
    irx = df.get('IRX', pd.Series(4.5, index=df.index)).fillna(method='ffill').fillna(method='bfill').fillna(4.5).to_numpy(dtype=float)

    if 'TNX' in df.columns:
        tnx = df['TNX'].fillna(method='ffill').fillna(method='bfill').fillna(irx + 1.0).to_numpy(dtype=float)
    else:
        tnx = irx + 1.0
    ...
    rv = (df['SPY_Ret'].rolling(20, min_periods=5).std().fillna(method='bfill').fillna(0.15 / np.sqrt(252)).to_numpy(dtype=float) * np.sqrt(252))
...
def simulate_latent_stress_state(n_days: int, regime_path: np.ndarray,
    ...
    for t in range(1, n_days):
        ...
        # Fix 2: dampen VIX amplification (was creating runaway feedback loop)
        vix_amp = max((vix_series[t] - 25.0) / 30.0, 0.0)
        # Fix 4: dampener on VIX→jump coupling; fix 2: hard-cap at 1%
        jump_prob = float(np.clip(base_prob + 0.05 * vix_amp + 0.02 * liquidity[t], 0.0, 0.01))
        if rng.random() < jump_prob:
            # Symmetric t draws: fat-tailed dislocations in both directions.
            # Negative draw = gap-down (crash), positive = gap-up (relief rally).
            # df=6 chosen: E[z^4] = 13.5 (finite, bounded) vs df=4 (E[z^4]=∞).
            # Sign asymmetry is handled by GJR-GARCH leverage effect (γ>0), not
            # by biasing jump signs (which would add negative drift bias to CAGR).
            jump[t] = rng.standard_t(df=6) * jump_scale
...
def generate_fat_tailed_returns(n_days: int, regime_path: np.ndarray,
    ...
    if sim_engine_mode == 'institutional_v1' and joint_return_model is not None:
        joint = simulate_joint_returns_t(n_days=n_days, regime_path=regime_path, joint_model=joint_return_model, rng=rng)
...
def generate_tracking_error_ar1(n_days, regime_path, vix_series, underlying_returns,
    ...
    for i in range(1, n_days):
        ...
        if liquidity_series is not None and i < len(liquidity_series):
            liq_mult += 0.20 * np.clip(liquidity_series[i], 0.0, 3.0)
        ...
        innovation *= base_te * vix_multiplier * regime_multiplier * liq_mult

        move_multiplier = (1.0 + 10.0 * abs(underlying_returns[i])) * downside_scale
        te_series[i] = rho * te_series[i-1] + np.sqrt(max(1 - rho**2, 1e-6)) * innovation * move_multiplier
...
def simulate_regime_path_semi_markov(total_days: int,
    ...
    while t < total_days:
        ...
        if current_regime in weibull_params:
            ...
            spell = max(1, int(round(raw_spell)))
...
def simulate_single_path_fixed(args):
    ...
    # Apply crisis_prob_scale to calm→crisis transition probability.
    # tm[0,1] is P(enter crisis | in calm).  Scale then renormalize row 0 so
    # the row remains stochastic: tm[0,0] = 1 - tm[0,1].  Row 1 is unchanged.
    # Steady-state impact: π₁ = tm[0,1]/(tm[0,1]+tm[1,0]) increases when
    # crisis_prob_scale > 1 (intended behavior — more time in crisis).
    # Cap at 0.20 to prevent unrealistic permanent-crisis lock.
    if _crisis_scale != 1.0:
        _tm = np.array(transition_matrix, dtype=float)
        _tm[0, 1] = float(np.clip(_tm[0, 1] * _crisis_scale, 0.0, 0.20))
        _tm[0, 0] = 1.0 - _tm[0, 1]   # row-stochastic: sum = 1 ✓
    ...
    if SIM_ENGINE_MODE == 'institutional_v1' and stress_state_model is not None:
        ...
        spy_returns = np.clip(spy_returns + jump, -0.95, 3.0)
        qqq_returns_raw = np.clip(qqq_returns_raw + 1.35 * jump, -0.95, 4.0)
    ...
    if _daily_equity_add != 0.0:
        spy_returns     = np.clip(spy_returns     + _daily_equity_add, -0.95, 3.0)
        qqq_returns_raw = np.clip(qqq_returns_raw + _daily_equity_add, -0.95, 4.0)
    ...
    for sid in strategies:
        try:
            equity_curve, trades = run_strategy_fixed(sim_df, sid, regime_path, correlation_matrices, apply_costs=True)
...
def main():
    ...
    regime_model['stress_state_model'] = calibrate_stress_state_model(df, regime_model['regimes_historical'])
    ...
    print("Running validation tests...")
    validation_results = run_validation_tests(df=df, regime_model=regime_model)
    ...
    _tm  = regime_model.get('transition_matrix')
    for _i in range(500):
        _rng_i = np.random.default_rng(_i + 77777)
        _n = 252
        _reg = np.zeros(_n, dtype=int); _r = 0
        for _t in range(1, _n):
            if _rng_i.random() > _tm[_r, _r]: _r = 1 - _r
            _reg[_t] = _r
        if _jrm is not None:
        ...
        if _ssm is not None:
            ...
            _spy = np.clip(_spy + _jmp, -0.95, 3.0)
