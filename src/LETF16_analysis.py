...
def create_summary_statistics(mc_results, time_horizon):
    ...
    print("="*100)
    print(f"{'Rank':<5} {'ID':<5} {'Strategy':<30} {'Median$':>12} {'CAGR':>8} {'Win%':>8} | {'MaxDD':>9} {'Trd/Y':>7}")
    print("-"*100)
    ...
    for sid in roth_ids:
        ...
        roth_data.append({
            'id': sid, 'median': median, 'cagr': cagr, 'win': win_rate,
    ...
    for i, d in enumerate(roth_data, 1):
        print(f"{i:<5} {d['id']:<5} {d['name']:<30} {d['median']:>12,.0f} {d['cagr']*100:>8.1f}% {d['win']:>8.1f}% | {d['dd']*100:>9.1f}% {d['trades']:>7.1f}")
    ...
    print(f"{'='*140}")
    print(f"{'Rank':<5} {'ID':<5} {'Strategy':<30} {'Median':>15} {'Tax':>10} {'Win%':>8} | {'MaxDD':>9} {'Trd/Y':>7}")
    print(f"{'':>5} {'':>5} {'':>30} {'Pre→Post':>15} {'Drag%':>10} {'':>8} | {'':>9} {'':>7}")
    print("-"*140)
    ...
    for sid in taxable_ids:
        ...
        # Calculate percentiles with tax
        pcts = {}
        for pname, pval in [('p10',10), ('p25',25), ('p40',40), ('p60',60), ('p75',75), ('p90',90)]:
    ...
    for i, item in enumerate(data, 1):
        med_pre = item['pcts']['p60']['pre_cagr'] * 100
        med_post = item['pcts']['p60']['post_cagr'] * 100
        drag = item['pcts']['p60']['drag']
        
        print(f"{i:<5} {item['id']:<5} {item['name']:<30} "
              f"{med_pre:>6.1f}%→{med_post:>5.1f}% {drag:>9.1f}% {item['win']:>8.1f}% | "
    ...
    for rank, item in enumerate(data[:5], 1):
        ...
        print("-"*140)
        print(f"{'':>10} {'P10':>12} {'P25':>12} {'P40':>12} {'P60':>12} {'P75':>12} {'P90':>12}")
        print("-"*140)
        ...
        for pn in ['p10', 'p25', 'p40', 'p60', 'p75', 'p90']:
            d = item['pcts'][pn]
            pre_cagr += f"    {d['pre_cagr']*100:>7.1f}%"
        print(pre_cagr)
        ...
        for pn in ['p10', 'p25', 'p40', 'p60', 'p75', 'p90']:
            d = item['pcts'][pn]
            post_cagr += f"    {d['post_cagr']*100:>7.1f}%"
        print(post_cagr)
        ...
        for pn in ['p10', 'p25', 'p40', 'p60', 'p75', 'p90']:
            d = item['pcts'][pn]
            drag_line += f"      {d['drag']:>7.1f}%"
        print(drag_line)
        
        print("-"*140)
        
        # Market scenarios for this strategy
        print("\nMARKET SCENARIOS:\n")
        ...
        for pval in [10, 25, 40, 60, 75, 90]:
            ...
            print(explain_percentile(pval, d['pre_cagr']*100, time_horizon, spy_cagr))
