...
def create_summary_statistics(mc_results, time_horizon):
    ...
    # --- NEW METRIC 6: Rank Strategies ---
    # Sort by Median Wealth (Descending)
    summary_data.sort(key=lambda x: x['Median'], reverse=True)
    
    # Print Table
    # Header
    print(f"{'Rank':<2} {'Strategy':<20} {'Median$':>8} {'CAGR':>5} {'Win%':>5} | "
          f"{'P10':>4} {'P25':>8} {'P40':>8} {'P60':>8} {'P75':>8} {'P90':>8} | "
          f"{'MaxDD':>6} {'Recov(Y)':>6} {'Trd/Y':>5}")
    print("-" * 140)
    
    for i, row in enumerate(summary_data):
        print(f"{i+1:<4} {row['Strategy']:<20} "
              f"{row['Median']:>8,.0f} {row['CAGR']*100:>6.1f}% {row['WinRate']:>5.1f} | "
              f"{row['P10']:>4,.0f} {row['P25']:>8,.0f} {row['P40']:>8,.0f} {row['P60']:>8,.0f} {row['P75']:>8,.0f} {row['P90']:>8,.0f} | "
              f"{row['Median_DD']*100:>6.1f}% {row['Recovery_Yrs']:>6.1f} {row['Trades_Yr']:>5.1f}")
