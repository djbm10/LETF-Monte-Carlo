...
def fetch_historical_data():
    ...
    # Final harmonization pass (after all filling)
...
def generate_tracking_error_ar1(
    n_days, regime_path, vix_series, underlying_returns,
...
# ========================================================================
    ...
    # NOTE: For leveraged ETFs, a single-day return can theoretically exceed -100%
    # (e.g., 3x leverage with -34% underlying = -102%). In reality, the fund would
    # halt trading or liquidate before NAV goes negative. We floor (1+r) at a tiny
    # positive value to prevent cumprod from flipping the price negative.
    for asset in assets_order:
        growth_factors = (1 + sim_df[f'{asset}_Ret'].fillna(0)).clip(lower=1e-8)
        sim_df[f'{asset}_Price'] = growth_factors.cumprod() * 100

    # Add TLT price and returns (unleveraged version of TMF)
    sim_df['TLT_Ret'] = sim_df['TMF_Ret'] / 3.0  # Unlever TMF to get TLT
    growth_factors_tlt = (1 + sim_df['TLT_Ret'].fillna(0)).clip(lower=1e-8)
    sim_df['TLT_Price'] = growth_factors_tlt.cumprod() * 100

    sim_df['VIX'] = vix
    if stress_state is not None:
        sim_df['Liquidity_Stress'] = stress_state.get('liquidity', np.zeros(sim_days))
        sim_df['Credit_Stress'] = stress_state.get('credit', np.zeros(sim_days))

    layer_checks = validate_simulation_layers(sim_df)
    if not layer_checks.get('is_valid', False):
        raise RuntimeError(f"Simulation layer validation failed: {layer_checks}")

    sim_df.attrs['simulation_metadata'] = build_simulation_metadata(
        sim_id=sim_id,
        regime_path=regime_path,
