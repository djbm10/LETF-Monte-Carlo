...
class BottleneckWinnerFreePIT(QCAlgorithm):
    ...
    Required project parameters:
        fundamentals_url=<SEC formation-date CSV generated by sec_formation_features.py>
            required for every model except pure momentum
        network_url=<CIK network CSV generated by sec_item1_network.py>
            required for bottleneck models
    ...
    def _load_sec_fundamentals(self, url):
        # cik -> sorted list of (formation_date, metrics)
        out = defaultdict(list)
        if not url:
            if self.model_name != "momentum":
                self.debug("No fundamentals_url supplied; SEC accounting features will be missing.")
            return out
        raw = self.download(url)
        if not raw:
            raise ValueError("fundamentals_url returned empty content")
