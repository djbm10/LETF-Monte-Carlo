...
# 2024 Tax Brackets (Single filer) - PROPER MARGINAL RATES
TAX_BRACKETS_2024 = [
...
LTCG_BRACKETS_2024 = [
    (47025, 0.00),      # $0 - $47,025: 0%
    (518900, 0.15),     # $47,026 - $518,900: 15%
    (float('inf'), 0.20) # $518,901+: 20%
]

STANDARD_DEDUCTION_2024 = 14600
NIIT_THRESHOLD_2024 = 200000  # 3.8% NIIT on investment income
NIIT_RATE = 0.038

# State tax (California as example)
CA_TAX_BRACKETS = [
    (10412, 0.01),
    (24684, 0.02),
    (38959, 0.04),
...
def create_summary_statistics(mc_results, time_horizon):
    """Generate summary table with Advanced Metrics"""
    ...
    # 1. Get SPY data first for "Win Rate" calculation
    spy_wealths = []
    if 'S2' in mc_results: # Assuming S2 is SPY
        spy_wealths = [r['Final_Wealth'] for r in mc_results['S2']]
    ...
    for sid in STRATEGIES.keys():
        ...
        max_dds = [r.get('Max_DD', 0) for r in results]
        median_dd = np.median(max_dds)
        
        # --- NEW METRIC 1: 25th & 75th Percentile ---
        p10 = np.percentile(wealth, 10)
        ...
        # --- NEW METRIC 2: Win Rate vs SPY ---
        # Compare simulation-by-simulation (paths are aligned by index)
        if len(spy_wealths) == len(wealth):
            wins = sum(w > s for w, s in zip(wealth, spy_wealths))
            win_rate = (wins / len(wealth)) * 100
        else:
            win_rate = 0.0

        # --- NEW METRIC 3: Accurate Trades ---
        # We take the mean of the exact trades per year tracked in simulation
        avg_trades_yr = np.mean([r.get('Trades_Per_Year', 0) for r in results])
        
        # --- NEW METRIC 5: Recovery Time ---
        # Convert days to years (assuming 252 trading days)
        underwater_days = [r.get('Max_Underwater_Days', 0) for r in results]
        median_recovery_yrs = np.median(underwater_days) / 252.0
        ...
        summary_data.append({
            ...
            'WinRate': win_rate,
            'Median_DD': median_dd,
            'Recovery_Yrs': median_recovery_yrs,
            'Trades_Yr': avg_trades_yr
        })
    
# --- CATEGORIZED OUTPUT LOGIC ---
    
    # Define your specific Roth-Safe IDs
    # Includes: TQQQ B/H, SPY B/H, SSO B/H, 200 SMA, SMA Band, 60/40, and Composite Regime
    roth_ids = ['S1', 'S2', 'S3', 'S4', 'S5', 'S6']

    # Define the two categories
    groups = [
        {
            "title": "ROTH IRA COMPATIBLE (Low Frequency / Cash Account Safe)",
            "subtitle": "Optimized for tax-free compounding with minimal settlement friction:",
            "filter": lambda x: x['ID'] in roth_ids
        },
        {
            "title": "TAXABLE BROKERAGE (High Frequency / Advanced Risk Management)",
            "subtitle": "Requires margin and generates significant short-term capital gains:",
            "filter": lambda x: x['ID'] not in roth_ids
        }
    ]

    print("\n" + "="*100)
    print("PORTFOLIO STRATEGY SEGMENTATION BY ACCOUNT TYPE")
    print("="*100)

    for group in groups:
        # Apply the filter for this specific account category
        group_data = [row for row in summary_data if group['filter'](row)]
        
        # Sort by Median Wealth (Best performing at the top)
        group_data.sort(key=lambda x: x['Median'], reverse=True)
        
        if not group_data:
            continue

        print(f"\n> {group['title']}")
        print(f"  {group['subtitle']}")
        print("-" * 100)
        
        # Header formatting
        header = (f"{'Rank':<5} {'ID':<4} {'Strategy':<25} {'Median$':>12} {'CAGR':>8} {'Win%':>8} | "
                  f"{'MaxDD':>9} {'Trd/Y':>7}")
        print(header)
        print("-" * 100)

        for i, row in enumerate(group_data):
            # Print row with clean formatting
            print(f"{i+1:<5} {row['ID']:<4} {row['Strategy']:<25} "
                  f"{row['Median']:>12,.0f} {row['CAGR']*100:>8.1f}% {row['WinRate']:>8.1f}% | "
                  f"{row['Median_DD']*100:>9.1f}% {row['Trades_Yr']:>7.1f}")
