...
def get_start_date_interactive() -> tuple:
    ...
    print(f"\nFull data available: {DATA_START_DATE} to {DATA_END_DATE} ({years_available:.1f} years)")
    ...
    print(f"  Duration: {years_selected:.1f} years")
...
def validate_time_horizons_for_start_date(start_date: str, time_horizons: list) -> list:
    ...
    if removed_horizons:
        ...
        print(f"  Available: {available_years:.1f} years ({start_date} to {ANALYSIS_END_DATE})")
    ...
    if removed_horizons:
        print(f"\n  Note: Some time horizons removed due to insufficient data:")
        print(f"  Available: {available_years:.1f} years from {start_date}")
        print(f"  Removed horizons: {removed_horizons}")
        print(f"  Valid horizons: {valid_horizons}")
...
# 20 days ≈ 1 month - preserves short-term patterns and volatility clustering
BOOTSTRAP_BLOCK_SIZE = 20

# Degrees of freedom for Student-t noise
# Lower = fatter tails, more extreme events
# df=5 is commonly used for equity returns (realistic fat tails)
# df=3 would be even more extreme (crisis scenarios)
STUDENT_T_DF = 5

# How much of the return comes from bootstrap vs Student-t noise
# 0.85 = 85% historical bootstrap + 15% Student-t noise
# This adds variation while preserving historical patterns
...
def fetch_historical_data():
    ...
    print(f"    Years of data: {(df.index[-1] - df.index[0]).days / 365.25:.1f}")
    ...
    print("DATA SUMMARY")
    print(f"{'='*80}")
    print(f"✓ Data ready: {len(df):,} trading days ({len(df)/252:.1f} years)")
...
class BlockBootstrapReturns:
    ...
    def _create_regime_blocks(self) -> Dict[int, List[np.ndarray]]:
        ...
        # Also get NASDAQ and Treasury returns if available
        if 'QQQ_Ret' in self.df.columns:
            qqq_ret = self.df['QQQ_Ret'].values
        else:
            qqq_ret = spy_ret * 1.25  # Approximate
    ...
    def sample_block(self, regime: int, rng: np.random.Generator = None) -> np.ndarray:
        ...
        Returns:
            Array of shape (block_size, 4) with columns [SPY, QQQ, TLT, VIX]
        """
        if rng is None:
    ...
    def _generate_synthetic_block(self, regime: int, rng: np.random.Generator) -> np.ndarray:
        ...
        if regime == 0:  # Low vol
            daily_std = 0.01  # ~16% annual vol
            vix_base = 15
        else:  # High vol
            daily_std = 0.025  # ~40% annual vol
            vix_base = 35
        ...
        return np.column_stack([spy_ret, qqq_ret, tlt_ret, vix])
    ...
    def sample_returns(self, n_days: int, regime_path: np.ndarray,
        ...
        if add_student_t_noise and bootstrap_weight < 1.0:
            ...
            # Get regime-dependent volatility for noise scaling
            noise_scale_spy = np.where(regime_path == 0, 0.005, 0.012)
            noise_scale_qqq = noise_scale_spy * 1.25
            noise_scale_tlt = noise_scale_spy * 0.5
            
            # Generate Student-t noise (fatter tails than normal)
            spy_noise = rng.standard_t(df=STUDENT_T_DF, size=n_days) * noise_scale_spy
            qqq_noise = rng.standard_t(df=STUDENT_T_DF, size=n_days) * noise_scale_qqq
            tlt_noise = rng.standard_t(df=STUDENT_T_DF, size=n_days) * noise_scale_tlt
            
            # Blend bootstrap returns with noise
...
def generate_fat_tailed_returns(n_days: int, regime_path: np.ndarray,
    ...
    for regime_id in range(N_REGIMES):
        ...
        # QQQ: Higher beta (1.25x SPY)
        qqq_returns[mask] = regime_returns * 1.25
...
# ========================================================================
    ...
    for asset in assets_order:
        ...
        # Calculate time-varying borrowing cost based on regime
        # This creates an array of daily borrowing costs
        ...
        if asset == 'TQQQ':
            ...
            if USE_BLOCK_BOOTSTRAP and qqq_returns_raw is not None:
                underlying = qqq_returns_raw * (beta / 1.25)  # Adjust for beta
            else:
...
def calculate_historical_rolling_cagrs(df: pd.DataFrame, asset: str, 
    ...
    if len(df_real) < window_days:
        print(f"  Warning: {asset}: Insufficient {data_type} data "
              f"({len(df_real)/252:.1f}Y available, {years}Y needed)")
        return None
...
def compare_simulated_vs_historical(df: pd.DataFrame, mc_results: Dict, 
    ...
    for sid, asset in strategy_to_asset.items():
        ...
        print(f"     CAGR Range: {hist_stats['min']*100:+.1f}% to {hist_stats['max']*100:+.1f}%")
        print(f"     CAGR Median: {hist_stats['median']*100:+.1f}%")
        print(f"     CAGR Std Dev: {hist_stats['std']*100:.1f}%")
        ...
        print(f"     CAGR Range: {sim_stats['min']*100:+.1f}% to {sim_stats['max']*100:+.1f}%")
        print(f"     CAGR Median: {sim_stats['median']*100:+.1f}%")
        print(f"     CAGR Std Dev: {sim_stats['std']*100:.1f}%")
        ...
        print(f"\n  PERCENTILE RANKINGS:")
        print(f"     Historical median ({hist_stats['median']*100:+.1f}%) would be P{hist_median_in_sim:.0f} in simulation")
        print(f"     Simulated median ({sim_stats['median']*100:+.1f}%) would be P{sim_median_in_hist:.0f} in history")
        ...
        print(f"     Median Difference: {median_diff*100:+.2f}% ({median_diff_pct:+.1f}% relative)")
        ...
        for pct_name in ['p5', 'p10', 'p25', 'p50', 'p75', 'p90', 'p95']:
            ...
            print(f"     {pct_name.upper():<12} {hist_val:>+11.1f}% {sim_val:>+11.1f}% {diff:>+11.1f}%")
        ...
        if abs(median_diff) < 0.03:
            quality_score += 25
            quality_notes.append(f"OK: Small median difference ({median_diff*100:+.1f}%)")
        elif abs(median_diff) < 0.05:
            quality_score += 15
            quality_notes.append(f"WARN: Moderate median difference ({median_diff*100:+.1f}%)")
        else:
            quality_notes.append(f"BAD: Large median difference ({median_diff*100:+.1f}%)")
    ...
    if comparison_results:
        ...
        print(f"  Average Distribution Overlap: {avg_overlap:.1f}%")
...
def calculate_rolling_cagrs_all_data(df: pd.DataFrame, asset: str, 
    ...
    if len(df_all) < window_days:
        print(f"  Warning: {asset}: Insufficient data "
              f"({len(df_all)/252:.1f}Y available, {years}Y needed)")
        return None
...
def compare_simulated_vs_synthetic_historical(df: pd.DataFrame, mc_results: Dict, 
    ...
    for sid, asset in strategy_to_asset.items():
        ...
        if config:
            ...
            print(f"     Tracking Error Base: {config.get('tracking_error_base', 0)*10000:.1f} bps")
        ...
        print(f"     - Periods with synthetic data: {all_stats['synthetic_count']} ({all_stats['synthetic_pct']:.1f}%)")
        ...
        print(f"     CAGR Range: {all_stats['min']*100:+.1f}% to {all_stats['max']*100:+.1f}%")
        print(f"     CAGR Median: {all_stats['median']*100:+.1f}%")
        print(f"     CAGR Std Dev: {all_stats['std']*100:.1f}%")
        ...
        print(f"\n  PERCENTILE RANKINGS:")
        print(f"     Historical+Synthetic median ({all_stats['median']*100:+.1f}%) would be P{all_median_in_sim:.0f} in simulation")
        print(f"     Simulated median ({sim_stats['median']*100:+.1f}%) would be P{sim_median_in_all:.0f} in historical+synthetic")
        ...
        for pct_name in ['p5', 'p10', 'p25', 'p50', 'p75', 'p90', 'p95']:
            ...
            print(f"     {pct_name.upper():<12} {all_val:>+11.1f}% {sim_val:>+11.1f}% {diff:>+11.1f}%")
        ...
        print(f"     Worst {time_horizon}Y period: {all_cagrs[worst_idx]*100:+.1f}% CAGR", end="")
        ...
        print(f"     Best {time_horizon}Y period:  {all_cagrs[best_idx]*100:+.1f}% CAGR", end="")
...
# ========================================================================
    ...
    if debug:
        ...
        print(f"    Estimated leverage: {estimated_leverage_ratio:.1f}x")
...
def main():
    ...
    print(f"  Total days: {len(df):,} ({len(df)/252:.1f} years)")
