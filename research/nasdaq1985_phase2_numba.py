...
PATHS=20_000; H=np.array([5,10,20,30,40,50],dtype=np.int64); WARM=300
...
@njit(cache=True)
def sim_batch(M,R):
    b,N=M.shape; S=len(NAMES); K=len(H); term=np.empty((b,S,K)); mdd=np.empty((b,S,K)); rec=np.empty((b,S,K))
    horizon_days=H*TD
    for p in range(b):
        px=np.empty(N); qld=np.empty(N); tq=np.empty(N)
        px[0]=1.; qld[0]=0.; tq[0]=0.
        # returns for synthetic vehicles; price is underlying index total-return proxy
        for t in range(N):
        ...
        # persistent states
        sma100_2=0.; sma100_25=0.; tv12=0.; tv0=0.; qhold=0.; thold=0.
        hk=0
        for t in range(WARM,N):
            sig=t-1
            # indicators known at prior close
            ...
            # strategy daily returns and approximate turnover cost on changing direct exposure.
            # For stateful strategies, prev exposure tracked via lastE.
            if t==WARM:lastE=np.zeros(S)
            rr=np.empty(S)
            for s in range(11):
                rr[s]=R[p,t]+E[s]*(M[p,t]-R[p,t])-0.00010*abs(E[s]-lastE[s])
            rr[11]=qhold*qld[t]-0.0003*abs(qhold-lastE[11])
            rr[12]=thold*tq[t]-0.0003*abs(thold-lastE[12])
            E[11]=qhold;E[12]=thold
            lastE=E.copy()
            for s in range(S):
                wealth[s]*=max(1.+rr[s],1e-12)
...
def bootstrap(arr_m,arr_r,dates,b,nlen,rng,sc):
    ...
    stress=np.isin(pd.DatetimeIndex(dates).year,[1987,2000,2001,2002,2008,2009,2020,2022])
    w=None
