...
def get(x,t):
 s=x[('Close',t)] if isinstance(x.columns,pd.MultiIndex) else x['Close'];s=pd.to_numeric(s,errors='coerce');s.index=pd.to_datetime(s.index).tz_localize(None);return s.pct_change().dropna()
...
def main():
 tick=['SPY','SSO','UPRO','QLD','TQQQ'];x=yf.download(tick,start='2006-01-01',end='2026-08-22',auto_adjust=True,progress=False);R={t:get(x,t) for t in tick};rows=[]
 ...
 # common-period apples-to-apples since TQQQ inception
