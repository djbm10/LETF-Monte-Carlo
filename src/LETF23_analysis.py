...
class WashSaleTracker:
    ...
    def __init__(self):
        ...
        # This tells us: "For the lot bought on replacement_buy_day, use original_lot_buy_day for holding period"
        ...
        for asset, trade_list in self.trades.items():
            ...
            for sell in sells:
                ...
                if wash_sale_buys:
                    ...
                    self.wash_sale_events.append(WashSaleEvent(
                        sale_day=sell_day,
                        asset=asset,
...
# ========================================================================
    ...
    # ========================================================================
    # We need to process wash sales BEFORE calculating gains/losses
    # because wash sales can disallow losses and adjust basis.
    #
    # Step 1: Record all trades in the wash sale tracker
    # Step 2: Process wash sales (identifies disallowed losses)
    # Step 3: Apply basis adjustments when calculating gains/losses
    ...
    # Process wash sales (this identifies all wash sales with look-forward)
    ...
    if debug:
        ...
        if ws_summary['events']:
            print(f"    First 5 events:")
            for event in ws_summary['events'][:5]:
                print(f"      Day {event['sale_day']}: ${event['loss_disallowed']:,.2f} disallowed "
                      f"(replacement buy on day {event['replacement_day']})")
    
    # ========================================================================
    # ORGANIZE TRADES BY YEAR WITH WASH SALE ADJUSTMENTS
    # ========================================================================
    ...
    for trade in trades:
        ...
        if trade['action'] == 'BUY':
            ...
            while shares_to_sell > 0.001 and positions[asset]:
                ...
                # Calculate gain/loss using ADJUSTED basis (includes wash sale adjustments)
                holding_days = sale_day - pos['original_day']  # Use original day for holding period
                
                # Use adjusted price (which includes any disallowed losses from prior wash sales)
                cost_basis = shares_sold * pos['adjusted_price']
                proceeds = shares_sold * sale_price
                gain_loss = proceeds - cost_basis
                
                # Check if THIS sale is a wash sale
                # We need to check if there's a buy within 30 days before OR after
                is_wash_sale = False
                if gain_loss < 0:  # Only losses can be wash sales
                    # Check all trades for buys within ±30 days
                ...
                if is_wash_sale and gain_loss < 0:
                    # Loss is disallowed - don't add to yearly losses
                    yearly_activity[year]['wash_sale_disallowed'] += abs(gain_loss)
                    # The basis adjustment was already applied to the replacement lot above
                else:
                    # Normal gain/loss processing
                    # Classify as ST or LT based on holding period
                    if holding_days > 365:
                        if gain_loss > 0:
