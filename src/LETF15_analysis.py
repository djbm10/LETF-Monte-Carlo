...
TAX_BRACKETS_2024 = FEDERAL_TAX_BRACKETS_2024['single']
LTCG_BRACKETS_2024_SINGLE = LTCG_BRACKETS_2024['single']
CA_TAX_BRACKETS = STATE_TAX_BRACKETS['CA']['single']
CA_STANDARD_DEDUCTION = STATE_TAX_BRACKETS['CA']['std_deduction']['single']
STANDARD_DEDUCTION_2024_SINGLE = STANDARD_DEDUCTION_2024['single']
NIIT_RATE = 0.038
...
def calculate_comprehensive_tax_v6(
    ...
    income_for_ltcg_brackets = total_income - STANDARD_DEDUCTION_2024
    
    federal_ltcg_tax = 0
    if taxable_lt > 0:
        # Find LTCG rate based on total income
        if income_for_ltcg_brackets <= LTCG_BRACKETS_2024[0][0]:
            ltcg_rate = LTCG_BRACKETS_2024[0][1]  # 0%
        elif income_for_ltcg_brackets <= LTCG_BRACKETS_2024[1][0]:
            ltcg_rate = LTCG_BRACKETS_2024[1][1]  # 15%
        else:
            ltcg_rate = LTCG_BRACKETS_2024[2][1]  # 20%
    ...
    if include_state:
        ...
        state_tax = calculate_marginal_tax(
            state_income,
            CA_TAX_BRACKETS,
    ...
    niit_tax = 0
    if include_niit and total_income > NIIT_THRESHOLD_2024:
        investment_income = taxable_st + taxable_lt
        niit_base = min(investment_income, total_income - NIIT_THRESHOLD_2024)
...
class TradeJournal:
    ...
    def log_allocation_change(self, day: int, asset: str, 
        ...
        delta_alloc = new_allocation - prev_allocation
        if abs(delta_alloc) < 0.001:
            return
        
        dollar_change = abs(delta_alloc * portfolio_value)
        action = 'BUY' if delta_alloc > 0 else 'SELL'
...
STRATEGIES = {
    ...
    'S3': {'name': 'SSO Buy & Hold (2x)', 'type': 'benchmark', 'asset': 'SSO'},
...
def get_tax_config_interactive():
    ...
    print("\nYour Ordinary Income (W-2, salary, etc.):")
    income_str = input("  Enter amount [default 100000]: ").strip() or "100000"
    try:
    ...
    print(f"  Income: ${ordinary_income:,}")
...
def explain_percentile(p, pre_cagr, horizon):
    """Market scenario explanation"""
    
    scenarios = {
        10: f"""
P10 - WORST 10% (You beat this in 90% of cases)
Pre-Tax CAGR: {pre_cagr:.1f}%

WHAT GOES WRONG:
• 2-3 major crashes (2008-level)
• VIX stays >30 for months
• Strategy whipsaws badly
• Worst {horizon}-yr period since Depression

Historical: 2000-2010 (tech+housing crashes)
Probability: 1 in 10
Planning: Don't plan on this, but prepare for it
""",
        25: f"""
...
Probability: 1 in 4
Planning: Reasonable downside case
""",
        40: f"""
...
Probability: Common
Planning: Reasonable optimistic case
""",
        75: f"""
...
def create_summary_statistics(mc_results, time_horizon):
    """NEW: Percentile-based analysis with tax customization"""
    ...
    print(f"\n{'='*180}")
    print(f"PERCENTILE ANALYSIS: {time_horizon}-YEAR TAXABLE BROKERAGE")
    print(f"{'='*180}")
    print(f"Tax: {TAX_CONFIG['state_name']} | ${TAX_CONFIG['ordinary_income']:,} | {TAX_CONFIG['filing_status'].title()}")
    print(f"{'='*180}\n")
    
    print("Percentile Guide:")
    print("  P10 = Worst 10% (beat 90% of time) | P25 = Worst 25% | P40 = Below median")
    print("  Median = 50th percentile | P60 = Above median | P75 = Best 25% | P90 = Best 10% (rare!)")
    print(f"{'='*180}\n")
    ...
    for sid in taxable_ids:
        ...
        for pname, pval in [('p10',10), ('p25',25), ('p40',40), ('median',50), ('p60',60), ('p75',75), ('p90',90)]:
            ...
            gains = pre_wealth - INITIAL_CAPITAL
            tax_result = estimate_tax_simple(gains, TAX_CONFIG['ordinary_income'], 
                                            TAX_CONFIG['state'], TAX_CONFIG['filing_status'])
            post_wealth = pre_wealth - tax_result['total_tax']
            pre_cagr = (pre_wealth / INITIAL_CAPITAL) ** (1/time_horizon) - 1
            post_cagr = (post_wealth / INITIAL_CAPITAL) ** (1/time_horizon) - 1
            drag = ((pre_cagr - post_cagr) / pre_cagr * 100) if pre_cagr > 0 else 0
            
            pcts[pname] = {'pre_cagr': pre_cagr, 'post_cagr': post_cagr, 'drag': drag,
                          'pre_wealth': pre_wealth, 'post_wealth': post_wealth}
        
        data.append({'id': sid, 'name': STRATEGIES[sid]['name'], 'pcts': pcts})
    
    data.sort(key=lambda x: x['pcts']['median']['post_wealth'], reverse=True)
    
    # Print main table
    print(f"{'Strategy':<30} {'P10':<20} {'P25':<20} {'P40':<20} {'Median':<20} {'P60':<20} {'P75':<20} {'P90':<20}")
    print(f"{'(by Median)':<30} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20} {'Pre→Post(Drag)':<20}")
    print("-" * 180)
    
    for item in data[:10]:
        row = f"{item['name'][:28]:<30}"
        for pn in ['p10', 'p25', 'p40', 'median', 'p60', 'p75', 'p90']:
            d = item['pcts'][pn]
            cell = f"{d['pre_cagr']*100:>4.1f}→{d['post_cagr']*100:>4.1f}({d['drag']:>2.0f}%)"
            row += f" {cell:<20}"
        print(row)
    
    print("="*180)
    print("\nTable Key: Pre→Post = Pre-tax CAGR → Post-tax CAGR | (Drag) = Tax drag %")
    print("="*180)
    ...
    for i, item in enumerate(data[:3], 1):
        print(f"{i}. {item['name']}")
        print("-"*180)
        print(f"{'Level':<12} {'Pre-Tax $':<18} {'Pre%':<10} {'Post-Tax $':<18} {'Post%':<10} {'Tax $':<15} {'Drag%':<10}")
        print("-"*180)
        
        for pn, pl in [('p10','P10'), ('p25','P25'), ('p40','P40'), ('median','Median'), 
                       ('p60','P60'), ('p75','P75'), ('p90','P90')]:
            d = item['pcts'][pn]
            tax_paid = d['pre_wealth'] - d['post_wealth']
            print(f"{pl:<12} ${d['pre_wealth']:>16,.0f} {d['pre_cagr']*100:>8.1f}% "
                  f"${d['post_wealth']:>16,.0f} {d['post_cagr']*100:>8.1f}% "
                  f"${tax_paid:>13,.0f} {d['drag']:>8.1f}%")
        print()
    ...
    for pval in [10, 25, 40, 60, 75, 90]:
        pname = f"p{pval}" if pval != 50 else 'median'
        if pname in best['pcts']:
            print(explain_percentile(pval, best['pcts'][pname]['pre_cagr']*100, time_horizon))
    ...
    print(f"{'='*180}")
    print("""
HOW TO USE THESE PERCENTILES:

CONSERVATIVE: Plan around P25-P40
  → Expect one major crash
  → Build emergency reserves
  → Don't be shocked if it happens

MODERATE: Plan around P40-P60 (median)
  → Normal market conditions
  → Most realistic assumption
  → Prepare for both sides

AGGRESSIVE: Hope for P60-P75, but prepare for P25
  → Don't bet everything on high percentiles
  → Keep emergency fund

NEVER PLAN ON P90: Only 10% chance
  → Requires perfect conditions
  → Planning around this is financial suicide

Remember: Pre-tax is fake. Post-tax is what you keep.
          Tax drag increases with success (higher brackets).
""")
    print("="*180 + "\n")
    
    # Quick Roth IRA summary
    print(f"{'='*120}")
    print(f"ROTH IRA (No Tax) - {time_horizon} Years")
    print(f"{'='*120}")
    print(f"{'Strategy':<30} {'P10':<15} {'Median':<15} {'P90':<15}")
    print("-"*120)
    
    roth_ids = ['S1', 'S2', 'S3', 'S6']
    roth_names = {'S1': 'TQQQ B&H', 'S2': 'SPY B&H', 'S3': 'SSO B&H', 'S6': '60/40 TQQQ/TMF'}
    
    for sid in roth_ids:
        if sid in mc_results and mc_results[sid]:
            w = np.array([r['Final_Wealth'] for r in mc_results[sid]])
            p10, med, p90 = np.percentile(w, [10,50,90])
            print(f"{roth_names[sid]:<30} ${p10:>13,.0f} ${med:>13,.0f} ${p90:>13,.0f}")
...
def process_trades_with_wired_engine(
    ...
    for trade in trades:
        ...
        if trade['action'] == 'BUY':
            ...
            positions[asset].append({
                'day': trade['day_index'],
                'shares': trade['dollar_amount'] / trade['price'],
                'price': trade['price']
            })
            
        elif trade['action'] == 'SELL':
            # FIFO: sell from oldest positions
            shares_to_sell = trade['dollar_amount'] / trade['price']
    ...
    for year in range(time_horizon_years):
        ...
        # FIXED: Proper marginal tax calculation (NOT flat rates)
        # Assume $100k ordinary income for taxable accounts
        tax_calc = calculate_comprehensive_tax_v6(
            taxable_st=result.taxable_st,
            taxable_lt=result.taxable_lt,
            capital_loss_deduction=result.capital_loss_deduction,
            ordinary_income=100000,  # Parameterizable
            include_state=True,
            include_niit=True
        )
        
        year_tax = tax_calc['total_tax']
        ...
        yearly_results.append({
            ...
            'lt_cf': lt_cf,
            'capital_loss_deduction': result.capital_loss_deduction,
            'federal_total': tax_calc['federal_total'],
            'state_tax': tax_calc['state_tax'],
            'niit_tax': tax_calc['niit_tax'],
