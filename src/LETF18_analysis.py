...
def create_summary_statistics(mc_results, time_horizon):
    ...
    # NEW: Calculate percentiles with WIRED TAX ENGINE
        pcts = {}
    
    # For each percentile, find the simulation closest to it and calculate tax properly
        for pname, pval in [('p10',10), ('p25',25), ('p40',40), ('p60',60), ('p75',75), ('p90',90)]:
            pre_wealth = np.percentile(wealth, pval)
        
        # Find the simulation result closest to this percentile value
            closest_idx = np.argmin(np.abs(wealth - pre_wealth))
            sim_result = results[closest_idx]
        
        # Get the actual trades from this simulation
            trade_list = sim_result.get('Trade_List', [])
        
            if trade_list and len(trade_list) > 0:
            # Use the WIRED TAX ENGINE to calculate proper tax
            ...
            # No trades = no tax (shouldn't happen but safe fallback)
                total_tax = 0
