...
# CALIBRATED to match historical+synthetic distribution
#
# Historical+Synthetic shows wider tails than pure bootstrap can produce
# because it includes scenarios like the Great Depression that may not
# repeat exactly, but SIMILAR magnitude events could occur.
#
# 0.70 = 70% historical bootstrap + 30% Student-t noise
# This allows for more extreme "what if" scenarios while preserving patterns
...
def fetch_historical_data():
    ...
    # DIAGNOSTIC: Check TQQQ price series
    if asset_id == 'TQQQ':
        print(f"\n  TQQQ DIAGNOSTIC:")
        print(f"    Price min: {df['TQQQ_Price'].min():.2e}")
        print(f"    Price max: {df['TQQQ_Price'].max():.2e}")
        print(f"    NaN count in QQQ_Ret: {df['QQQ_Ret'].isna().sum()}")
        print(f"    NaN count in TQQQ_Ret: {df['TQQQ_Ret'].isna().sum()}")
    # Sample prices
        for year in [1930, 1940, 1950, 1970, 1985, 2000, 2020]:
            mask = df.index.year == year
            if mask.any():
                p = df.loc[mask, 'TQQQ_Price'].iloc[0]
                print(f"    Price at {year}: {p:.2e}")
...
class BlockBootstrapReturns:
    ...
    def sample_returns(self, n_days: int, regime_path: np.ndarray,
        ...
        if add_student_t_noise and bootstrap_weight < 1.0:
            ...
            # CALIBRATED to match historical+synthetic TQQQ distribution
            # 
            # Historical+Synthetic TQQQ has:
            #   - Std Dev: 22.7% CAGR
            #   - P5: -51.7%, P10: -40.6%, P95: +40.0%
            #
            # To match this, we need more extreme daily moves:
            #   - Low vol: ~0.8% daily noise (was 0.5%)
            #   - High vol: ~2.5% daily noise (was 1.2%)
            #
            # CALIBRATED to produce realistic CAGR distributions:
            #   - SPY: ~4.5-5% CAGR std dev
            #   - TQQQ: ~22-23% CAGR std dev (needs ~1.5x QQQ multiplier, not 1.3x)
            #
            # The 3x leverage amplifies QQQ volatility significantly
            # Historical TQQQ has CAGR std dev of 22.7%, which requires
            # QQQ to have ~1.5x SPY volatility in the NOISE component
            noise_scale_spy = np.where(regime_path == 0, 0.008, 0.025)
            noise_scale_qqq = noise_scale_spy * 1.50  # Increased from 1.30 to match TQQQ distribution
